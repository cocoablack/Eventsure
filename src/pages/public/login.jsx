...
function LoginPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      navigate(requestedPath?.startsWith(`/${user.role}/`) ? requestedPath : roleHome(user.role), { replace: true });
    } catch (requestError) {
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#f0f2f5] px-4 pt-28 pb-10 sm:px-6 lg:px-20">
    ...
    <div className="login-image relative hidden min-h-[620px] bg-cover bg-center lg:block">
      ...
      <div className="absolute bottom-12 left-10 right-10 z-10">
        ...
        <p className="mt-4 max-w-lg text-lg leading-8 text-white/90">
          Your gateway to a curated marketplace of premium event professionals
      ...
      <form onSubmit={handleSubmit} className="mt-10 w-full max-w-lg">
        ...
        <div>
          ...
          <div className="mt-2 flex h-13 items-center rounded-xl bg-slate-100 px-4">
            <User size={22} className="mr-3 shrink-0 text-teal-800" />
            <input
              type="text"
              name="email"
              id="email"
              ...
              className="w-full bg-transparent text-sm outline-0 placeholder:font-medium"
            />
          </div>
        </div>
        ...
        <div className="mt-6">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <span className="text-sm text-slate-500" title="Password reset is not enabled yet">Password help: contact support</span>
          </div>

          <div className="mt-2 flex h-13 items-center rounded-xl bg-slate-100 px-4">
            <Lock size={22} className="mr-3 shrink-0 text-teal-800" />
            <input
              type="password"
              name="password"
              id="password"
        ...
        <button
          type="submit"
          disabled={submitting}
          className="mt-10 flex w-full items-center justify-center gap-2 rounded-xl bg-teal-800 px-5 py-4 text-base font-black text-white shadow-lg shadow-teal-900/20 hover:bg-teal-900"
        >
          {submitting ? "Signing in…" : "Login"} <ArrowRight size={22} />
        </button>
        ...
        <div className="rounded-2xl bg-slate-100 px-5 py-4">
          <p className="text-center text-sm italic leading-6 text-slate-600">
            Planning an event? Or providing a service? Our system automatically
            directs you to your specialized dashboard.
          </p>
        </div>
